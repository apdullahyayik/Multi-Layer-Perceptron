...
% mlpgm: türevsel azalma ve momentum kullanan çok katmanlý sinir aðý eðitimi iþlemi
%
%
%Çýkýþ Parametreleri
%         W: Giriþ ve hesaplama yapýlan katmanlara ait optimize edilmiþ aðýrlýklar
%         bias: iç çarpýmýn sýfýr olmasýný engelleyen parametre
%         noronSayisi: Ara katmanlarýn nöron sayýlarý dizisi
%
%Giriþ Parametreleri
%         egitimVeri: egitimde kullanýlacak olan veri (doðrulama verisi bu veri üzerinden alýnarak oluþturuluyor)
%         egitimHedef: egitimVeri ye ait sýnýf bilgileri
%         ogrenmeOrani: türevsel azalma iþlemindeki adým miktarý
%         momentum: Aðýrlýk deðiþimi için kullanýlan optimizasyon parametresi
%         noronSayisi: Ara katman nöron sayýlarý dizisi
%         iterasyonSayisi : Aðýrlýk optimzasyonu için tüm eðitim verisinin en fazla eðitilme sayýsý
%         sabirSiniri: Eðitimi erken durdurma için belirlenen, doðrulama verisine ait ortalama toplam hata karelerin sürekli azaldýðý iterasyon sayisi.
%         gosterim: Her iterasyon sonunda egitim verileri ile yapýlan aðýrlýk optimazasyonlarý sonrasý bu aðýrlýklar ile eðitim verileri ve
%         doðrulama verilerine ait ortalama hata karelerin grafiksel gösteriminin canlý olarak izlenmesi. (1 açýk, 0 kapali)
%
%
% Örnek Kullaným
...
%     error('Belirlenen katman sayisi ve aktivasyon fonksiyonlarý sayýsý uyumsuz.')
% end
...
switch gosterim
    case 1
        figure,xlabel('iterasyon sayisi'), ylabel('ortalama hata kareler toplamý'), goodplot, hold on,
end
        ...
        for di=length(noronSayisi)+1:-1:1
            switch di
                case length(noronSayisi)+1
                    ...
                    deltaW=(ogrenmeOrani.*delta{m, di}*H{1,di-1}'); % Doðal olarak son katmanda momentum uygulanamaz !
...
%                     deltaW=momentum.*(ogrenmeOrani.*delta{m, di}*I')+(ogrenmeOrani.*delta{m, di}*I'); % hýz
                    W{m+1,di}=W{m,di}+deltaW;
                otherwise
...
%                     deltaW=momentum.*(ogrenmeOrani.*delta{m, di-1}*H{1,di-1}')+(ogrenmeOrani.*delta{m, di}*H{1,di-1}');
    ...
    SSErr=0;
    SSErrval=0;
    % Eðitim verileri için Hata Kareler Toplamý (SSE)
    for i=1:egitimveriSayi
        D = egitimHedef(i,:)';
        I = egitimVeri(i,:)';
        for li=1:length(noronSayisi)+1
    ...
    MSSE(m)=SSErr/egitimveriSayi; % MSSE: Eðitim verilerinin hata karaler toplamý ortalamasý
    switch gosterim
        case 1
            plot(m,MSSE(m),'g+'), hold on
    end
    % Doðrulama verileri için Hata Kareler Toplamý (SSEval)
    ...
    MSSEval(m)=SSErrval/dogrulamaveriSayisi;  % MSSEval: Doðrulama verilerinin hata karaler toplamý ortalamasý
    switch gosterim
        case 1
            plot(m,MSSEval(m),'r*'), legend('egitim', 'doðrulama')
            pause(.00001)
    end
    % Erken durdurma
    if m>1
...
switch gosterim
    case 0
        figure,goodplot, plot(MSSE,'g+'), hold on, plot(MSSEval,'r*'), xlabel('iterasyon sayisi'), ylabel('ortalama hata kareler toplamý')
        legend('egitim', 'doðrulama')
end
% eðitimde baþarý
for i=1:egitimveriSayi
    % Eðitim verisi
